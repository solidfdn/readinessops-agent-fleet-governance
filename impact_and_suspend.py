...
AGENT_ID = "case-triage-agent"

if len(sys.argv) != 2:
    raise SystemExit("Usage: uv run python impact_and_suspend.py REVISION_ID")
...
# Fail closed: suspension test must start from a genuinely READY state.
...
# Ensure this Draft was created from the exact READY baseline.
if rev.get("parent_revision_id") != agent.get("current_revision_id"):
    raise ValueError("Draft parent does not match READY revision.")

if rev.get("base_publication_id") != agent.get("readiness_publication_id"):
    raise ValueError("Draft publication does not match READY publication.")
...
- If a previously verified mandatory safety capability is lost or can no
  longer be verified, classify this as MATERIAL and recommend SUSPEND.
...
if not impact.material_change:
    raise ValueError("Material change not established. Agent will not be suspended.")

if impact.treatment != "SUSPEND":
    raise ValueError("Impact does not require suspension.")
...
batch.set(
    db.collection("audit_events").document(suspend_audit_id),
    {
        ...
        "actor_type": "SYSTEM",
        "actor": "readinessops_safety_controller",
        "created_at": now,
    },
)
