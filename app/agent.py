...
def validate_grounding_claims(evidence: str, draft: str) -> str:
    """Deterministically validates measured facts and URLs before persistence."""
    issues = []

    def normalize_numbers(text: str) -> set[str]:
        found = re.findall(r"\$?\d+(?:,\d{3})*(?:\.\d+)?%?", text or "")
        return {re.sub(r"[$,%]", "", x) for x in found}
    ...
    def walk(value):
        if isinstance(value, dict):
            # Measured values must be supported by source evidence.
            ...
            # Evidence-defined targets must also exist in evidence.
            ...
            for token in value.split():
                ...
                if candidate.startswith(("http://", "https://")):
                    if candidate not in evidence:
                        issues.append(f"UNSUPPORTED_URL: {candidate}")
    ...
    result = {
        ...
        "rule": (
            "Measured values and EVIDENCE_DEFINED targets must be grounded "
            "in source evidence. Proposed targets are allowed when explicitly "
            "marked PROPOSED_TARGET."
        ),
    }
